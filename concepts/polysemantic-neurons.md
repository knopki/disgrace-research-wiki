...
Neurons that respond to multiple unrelated features. The counterpart is **monosemantic neurons**, which respond to a single interpretable feature. Both types can coexist in the same model, and the ratio between them is governed by feature sparsity. ^[raw/articles/toy-models-superposition/index.md]
...
In the ReLU hidden layer toy model, there is a clear transition: ^[raw/articles/toy-models-superposition/index.md]
...
Three empirical observations from real neural networks that are consistent with the toy model: ^[raw/articles/toy-models-superposition/index.md]
