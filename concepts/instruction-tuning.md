...
- **[[self-instruct|Self-Instruct]]** — contemporaneous method for bootstrapping instruction-tuning data from the model's own generations (Wang et al., 2022); addresses the data bottleneck that instruction tuning faces by generating 52k diverse instructions from only 175 seed tasks, eliminating the need for curated human datasets; demonstrated on GPT-3 at 175B scale
