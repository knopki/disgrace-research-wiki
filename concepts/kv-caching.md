...
- **[[multi-query-attention|Multi-Query Attention (MQA)]]** — the architectural origin of the KV cache memory-bandwidth analysis: Shazeer (2019) identified that incremental decoding is memory-bound because K and V tensors must be reloaded at every step. MQA reduces the KV cache size by a factor of `h` (number of heads) by sharing keys and values across heads, achieving 12× faster decoder throughput on TPUv2. This paper provides the theoretical foundation for understanding why KV caching is memory-bandwidth bound.
