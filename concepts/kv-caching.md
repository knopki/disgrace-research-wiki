...
- **[[multi-query-attention|Multi-Query Attention (MQA)]]** — the architectural origin of the KV cache memory-bandwidth analysis: [Shazeer (2019)](raw/papers/2019-11-shazeer-multi-query-attention/shazeer2019multiquery.md) identified that incremental decoding is memory-bound because K and V tensors must be reloaded at every step. MQA reduces the KV cache size by a factor of `h` (number of heads) by sharing keys and values across heads, achieving 12× faster decoder throughput on TPUv2. This paper provides the theoretical foundation for understanding why KV caching is memory-bandwidth bound.
...
- **[[flash-attention|FlashAttention]]** — accelerates the per-step attention computation via IO-aware tiling, making dense attention practical for long sequences. KV caching and FlashAttention are complementary and typically used together in production.
- **[[paged-attention|PagedAttention & vLLM]]** — the serving-system layer that actually *manages* the KV cache described above: instead of one contiguous tensor per request, the cache is paged into non-contiguous GPU blocks (OS virtual-memory analogy) to eliminate fragmentation and enable block-level sharing across requests. PagedAttention is the reason the KV-cache memory wall can be softened at serving time without changing the model.
- **[[speculative-decoding|Speculative Decoding]]** — orthogonal and complementary inference optimisation. KV caching avoids recomputation across decoding steps; speculative decoding reduces the number of serial model invocations by generating multiple tokens per parallel verification pass. They combine naturally: the verification step benefits from KV cache, and speculative decoding's parallel runs can share cached states.
- **[[switch-transformer|Switch Transformer]]** — MoE architectures introduce additional memory pressure (all expert parameters must be resident in GPU memory), compounding the KV cache memory wall problem. The two memory demands compete for the same HBM budget.
...
- **[[streaming-llm|StreamingLLM]]** — a training-free remedy for the KV cache growth problem in *streaming* (infinite-length) generation: instead of caching all past K/V, it keeps only a few "attention sink" initial tokens plus a fixed-size rolling window, holding constant memory while preserving stable attention. Directly addresses the linear-memory-growth constraint above without retraining.
